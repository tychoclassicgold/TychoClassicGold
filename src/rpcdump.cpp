// Copyright (c) 2009-2012 TychoClassicGold Developers
...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <tychoclassicgoldprivkey> [label] [rescan=true]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <tychoclassicgoldaddress>\n"
            "Reveals the private key corresponding to <tychoclassicgoldaddress>.");
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by TychoClassicGold %s (%s)\n", CLIENT_BUILD.c_str(), CLIENT_DATE.c_str());
